package ichen.chu.simplefingergesturelibs;
...
public class SimpleFingerGestures implements View.OnTouchListener {
    ...
    public boolean onTouch(View view, MotionEvent ev) {
        if (debug) mLog.d(TAG, "onTouch");
        switch (ev.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                if (debug) mLog.d(TAG, "ACTION_DOWN");
                startTracking(0);
                ga.trackGesture(ev);
                if (consumeTouchEvents) onFingerGestureListener.onTouch(view, ev);
                return consumeTouchEvents;
            case MotionEvent.ACTION_UP:
                if (debug) mLog.d(TAG, "ACTION_UP");
                if (tracking[0]) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                if (debug) mLog.d(TAG, "ACTION_POINTER_DOWN" + " " + "num" + ev.getPointerCount());
                startTracking(ev.getPointerCount() - 1);
                ga.trackGesture(ev);
                if (consumeTouchEvents) onFingerGestureListener.onTouch(view, ev);
                return consumeTouchEvents;
            case MotionEvent.ACTION_POINTER_UP:
                if (debug) mLog.d(TAG, "ACTION_POINTER_UP" + " " + "num" + ev.getPointerCount());
                if (tracking[1]) {
                    doCallBack(ga.getGesture(ev));
                }
            ...
            case MotionEvent.ACTION_CANCEL:
                if (debug) mLog.d(TAG, "ACTION_CANCEL");
                return true;
            case MotionEvent.ACTION_MOVE:
                if (debug) mLog.d(TAG, "ACTION_MOVE");
                if (consumeTouchEvents) onFingerGestureListener.onTouch(view, ev);
                return consumeTouchEvents;
        }
        return consumeTouchEvents;
    ...
    private void doCallBack(GestureAnalyser.GestureType mGt) {
        if (debug) mLog.d(TAG, "doCallBack, mGt.getGestureFlag()= " + mGt.getGestureFlag());
